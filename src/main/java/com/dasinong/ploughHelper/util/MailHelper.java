...
public class MailHelper {
	...
	private static HttpMethod postMethod(String target, String subject, String content) throws IOException{
		...
		NameValuePair[] param = { 
				new NameValuePair("api_user", apiUser),
				new NameValuePair("api_key", apiKey),
				new NameValuePair("from", "service@sendcloud.im"),
				new NameValuePair("fromname", "今日农事"),
				new NameValuePair("to", target),
				new NameValuePair("subject", subject),
				new NameValuePair("html", content),
				new NameValuePair("resp_email_id", "true")
		};
	...
	public static void main(String[] args){
		...
		mh.sendMail("coordinate6467@hotmail.com","来自SendCloud的第一封邮件！", "你太棒了！你已成功的从SendCloud发送了一封测试邮件，接下来快登录前台去完善账户信息吧！");
	}
}
