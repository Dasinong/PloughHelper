...
public class AllSystemMessage {
	...
	private AllSystemMessage() {
		...
		try {
			...
			while (result.next()) {
				SystemMessage sm = new SystemMessage(result.getInt("id"), result.getInt("monitorLocation"),
						result.getString("locationGroup"), result.getString("channel"), result.getString("content"),
						result.getDate("startTime"), result.getDate("endTime"), result.getString("picUrl"),
						result.getString("landingUrl"), result.getInt("institutionId"));
		...
		if (!isLoadFromDb) {
			...
			try {
				...
				sql = "insert into systemMessage (monitorLocation,locationGroup,channel,content,startTime,endTime,picUrl,landingUrl) values ("
						+ sm.getMonitorLocation() + "," + sm.getLocationGroup() + "," + sm.getChannel() + ","
						+ sm.getContent() + "," + sm.getStartTime() + "," + sm.getEndTime() + "," + sm.getPicUrl() + ","
						+ sm.getLandingUrl() + ")";
				stmt.execute(sql);
				stmt.close();
				conn.close();
