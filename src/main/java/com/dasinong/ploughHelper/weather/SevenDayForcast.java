...
public class SevenDayForcast {
	...
	public void addRawData(Date forecast_time, short weather, double temp,
				double max_temp, double min_temp, short ff_level,
				short dd_level, double rain){
		ForcastInfo newData = new ForcastInfo(forecast_time,weather, temp, max_temp, min_temp, ff_level,
				 dd_level,  rain);
		if (rc==0){
			rawData[rc] = newData;
			aggregateData[ac] = new ForcastInfo(forecast_time,weather, temp, max_temp, min_temp, ff_level,
			                   				 dd_level,  rain);
			rc++;
		}
		else{
