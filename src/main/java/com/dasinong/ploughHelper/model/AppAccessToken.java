 ...
 *   AppAccessToken token = manager.generate(appId);
 *   AppAccessToken token = manager.parse("13|KyeogIBfJIsDyOaH8c2IOA==");
 *   
 * For security reason, AppAcessToken should never ever be hard coded into 
 * client side, doing so would give everyone who decompiled your app the 
 * ability to modify your app. This implies that most of the time, you will 
 * be using app access tokens only in server to server calls.
 *   
 */
